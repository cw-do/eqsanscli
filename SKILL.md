...
How an agent drives `eqsanscli`, the interactive CLI for EQSANS data reduction at
SNS. One document covers both front ends: the Textual TUI a person sits in front
of, and the headless JSON protocol an agent speaks over stdin/stdout. Commands,
rules and outputs are identical — only the transport differs.

*(This file absorbed the former `AGENT_SKILL.md`, which had drifted: the two
hand-maintained copies each documented commands the other had lost.)*
...
Commands are the same in both. All start with `/`; anything without one goes to
the built-in LLM for translation, which an agent should not rely on — **always
send explicit `/commands`**. Commands are synchronous: wait for the result before
sending the next.
...
Typed at the `eqsans>` prompt. Long-running commands (`/reduce`, `/autopilot`)
show progress and can be cancelled with `Ctrl+X`.

### Headless — JSON over stdin/stdout

The mode to use when driving the tool programmatically.
...
**Finding the right run number:** `/show catalog` and read the **Class** column —
`S` scattering, `T` transmission, `BkgS`/`BkgT` background, `EmpT` empty beam,
`N` ignored. If a run is classified wrongly, `/reclass <run> <class>` fixes it at
the source; classification is what `/matchruns` pairs on, so fix it there rather
than assigning runs by hand.

**A run being empty beam is a *classification*, not an assignment.** "Run 186517
is the empty beam for 4m10a" means `/reclass 186517 empty` then `/matchruns` — not
`/set`. `/matchruns` pairs each run to its configurations from the ONCat distance
and wavelength.

#### Step 3: Presets and instrument files — automatic, verify only

`/matchruns` **already applied** the matching JSON preset for each new
configuration, and then resolved the cycle-specific calibration files (dark,
sensitivity, flux and the AgBe `detectoroffset` / `scalecomponents` /
`sampleoffset`) from the machine-physics folders by run number. Its output reports
both.

Verify rather than repeat:

```
/show config <id>          # effective parameters, with their source
/instrument show           # what the machine-physics resolver chose, and why
```

Only reach for `/apply preset auto` when a configuration reports **no matching
preset** and you have been given parameters to apply, or `/apply preset <name>
<cfg> --force` to overwrite deliberately. Re-running it is not a fix for anything
`/matchruns` has already done, and `--force` will overwrite user edits.

If a configuration reports no preset AND no instrument files, say so — reduction
will use drtsans defaults, which is rarely what the user wants.
...
| Command | Purpose |
|---------|---------|
| `/continue` | Resume previous session (cwd autosave, or the last one recorded anywhere) |
| `/session save [name]` | Save session (name optional) |
| `/session load <name>` | Load named session |
| `/session list` | Saved sessions, newest first, with date and IPTS |
...
| Command | Purpose |
|---------|---------|
| `/mask create <run> [options]` | Build a mask from that run's own detector image |
| `/mask list` | Masks discoverable from here |

The run number alone is enough — the archive is searched for it, so no `/load
ipts` is needed first. `--dry-run` reports and writes the preview without writing
the mask. See `knowledge/protocol.md` (MSK rules) for what a mask must satisfy.

### Multiple working tables

The session can hold several named tables — useful for keeping a standard sample
apart from the samples it calibrates.

| Command | Purpose |
|---------|---------|
| `/table` | Show the active table |
| `/table <name>` | Switch to (or create) a named table |
| `/move <rows> <table>` | Move rows out of the active table into another |
| `/list tables` | Saved tables on disk, newest first |

### Notes and comparison

| Command | Purpose |
|---------|---------|
| `/note add "<text>"` | Append to `NOTE.md` in the output directory |
| `/note show [N]` | Last N entries |
| `/compare <a> <b>` | Side-by-side parameter diff of two configs or presets |

Every successful command is auto-logged to `NOTE.md` (read-only commands
excepted), so replaying it reproduces the reduction.
...
| `/set drtsans <default\|dev\|qa>` | Which drtsans build reductions run under |
| `/models` | LLM models available for natural-language translation |
| `/ls [path]`, `/pwd`, `/cat <file>`, `/head <file> [n]`, `/tail <file> [n]` | Read-only filesystem access |

Write-capable shell commands (`/cp`, `/mv`, `/rm`, `/mkdir`, `/cd`) exist but are
deliberately not part of the agent workflow — reduction outputs are managed by
`/set outputdir`, `/share` and `/zipnsend`.
...
Two more failure signatures, from the TUI side:

| Symptom | Diagnosis | Fix |
|---------|-----------|-----|
| Wrong absolute scale | porsil calibration not applied | `/calibrate <porsil_Iq.dat> --applynow` |
| Stitched curves do not overlap | bad overlap range or target | `/stitch smart`, or `/stitch set <sample> overlap <q1> <q2>` |
