"""Decide what to mask on the EQSANS detector, and write the mask drtsans reads.

The **policy** layer: given a run's image, where is the beam stop, which tube
ends are dead, which tubes are faulty, and what should therefore be masked.
Detector geometry and image primitives it builds on — reshaping, pixel
positions, local contrast, cross cuts — live in `detector.py`; locating a run
file lives in `run_files.py`.
...
2. the **top and bottom pixel bands**, where response falls away at the tube ends
   (the long-standing convention is pixels 1-11 and 246-256, 1-based);
3. **deviant tubes** — judged against neighbours in their own front/back pack of
   four, because that stagger is by design and must not be read as a fault.

The rules a mask must satisfy are `MSK-01`..`MSK-11` in `knowledge/protocol.md`.

Everything here is plain numpy and unit-testable without Mantid. Only the two
steps that genuinely need it — reading a run's counts, and writing the mask NeXus
that drtsans consumes — are shelled out to the `drtsans` command, the same
mechanism `integrations/drtsans_runner.py` already uses for reductions.
"""
...
from eqsanscli.services.detector import (  # noqa: E402  (geometry primitives)
    CUT_BAND_MM, N_PIXELS, N_TUBES, TUBE_PACK, cut_along_x, cut_along_y,
    index_ticks, local_contrast, physical_tube_order, pixel_area_mm2,
    pixel_pitch_y_mm, reshape_counts, valley_walls,
)
