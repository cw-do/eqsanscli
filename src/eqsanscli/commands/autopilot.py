...
_USAGE = (
    ...
    "  13. Plot results\n"
    "\n"
    "--from N: skip steps 1..(N-1) and start at step N. Needs a loaded catalog\n"
    "  (from /load ipts). --from 2 starts at match-runs and BUILDS the table, so it\n"
    "  needs only the catalog; --from 3+ skip matching and need the table already\n"
    "  populated (e.g. from a prior /matchruns + /apply preset).\n"
    "  Examples:\n"
    "    --from 2  — skip catalog load; match runs and continue (only needs /load ipts)\n"
