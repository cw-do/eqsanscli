...
class EQSANSApp(App):
    ...
    def on_mount(self) -> None:
        ...
        logo = (
            ...
            "    0. [cyan]/oncat login[/cyan]             Sign in to ONCat (first time; approve a URL in your browser)\n"
    ...
    async def _handle_help(self, args: list[str], state: SessionState) -> CommandResult:
        ...
        if args and args[0].lower() in ("--simple", "-s", "simple", "quickstart"):
            simple_text = (
                ...
                "\n"
                "[bold cyan]0. Sign in to ONCat (first time only)[/]\n"
                "   [yellow]/oncat login[/]                     Approve the URL in your browser (copy it over SSH)\n"
                "   Per-user: you see only IPTS you can access. The token is cached and reused,\n"
                "   and first launch usually signs you in automatically — /oncat status to check.\n"
                "\n"
        ...
        help_text = (
            ...
            "\n"
            "[bold cyan]ONCat Sign-in (per-user):[/]\n"
            "  /oncat login                  — Sign in to ONCat (approve a URL in your browser; works over SSH)\n"
            "  /oncat status                 — Show whether you are signed in\n"
            "  /oncat logout                 — Remove the cached ONCat token\n"
            "  [dim]Access is per-user: you see only IPTS you can access. First use signs you in\n"
            "   automatically; the token is cached in ~/.eqsanscli/ and reused. Over SSH, copy the\n"
            "   sign-in URL into the browser on your own machine.[/]\n"
            "\n"
            "[bold cyan]Catalog & Data Loading:[/]\n"
            "  /load ipts <number>           — Fetch catalog from ONCat (only IPTS you can access)\n"
            ...
            "\n[bold cyan]Reduction preflight:[/]\n"
            "  [dim]/reduce refuses rows with no empty beam (it supplies the beam centre).[/dim]\n"
            "  /reduce <rows> --skip-missing  — reduce the valid rows, skip the incomplete ones\n"
            "  /reduce <rows> --force        — reduce anyway (expect drtsans failures)\n"
            "\n[bold cyan]Masks:[/]\n"
            "  /mask create <run>            — Build a mask from a run (banjo/flood/empty cell);\n"
            "                                  writes mask_<config>_<run>.nxs into the current folder\n"
            "  /mask create <run> --dry-run  — Preview PNG only, no mask file\n"
            "  /mask create <run> --beam-radius <mm>  — State the beam stop when detection is unsure\n"
            "  /mask create <run> --tubes 146         — Add a known-bad tube\n"
            "  /mask list                    — Masks discoverable from here\n"
            "  [dim]/mask (no args) for the full option list and troubleshooting.[/dim]\n"
    ...
    _GUIDE_TEXT = (
        ...
        "\n"
        "[bold cyan]0. Sign in to ONCat[/]\n"
        "   [yellow]/oncat login[/]\n"
        "   [dim]first time only; approve the\n"
        "   URL in your browser (per-user)[/]\n"
        "\n"
